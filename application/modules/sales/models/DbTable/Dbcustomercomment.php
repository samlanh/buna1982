<?php
class Sales_Model_DbTable_Dbcustomercomment extends Zend_Db_Table_Abstract
{
    protected $_name = 'tb_customer_comment';
    ...
    public function addcomment($_data){
	  $this->_name='tb_customer_comment';
 ...
 {
 	$db = $this->getAdapter();
 	$sql = "SELECT  * FROM tb_customer_comment WHERE id=$id ";
...
public  function getCustomerComment($search=null){
  $db = $this->getAdapter();
  $result = $this->getUserInfo();
  $sql = "SELECT id,
	(SELECT cust_name FROM `tb_customer` WHERE tb_customer.id=tb_customer_comment.customer_id LIMIT 1 ) AS customer_name,
	`comment`,date ,
	(SELECT fullname  FROM `tb_acl_user`
	WHERE `tb_acl_user`.`user_id`=`tb_customer_comment`.`user_id`) 
	AS `user`,
	(SELECT `name_en` FROM `tb_view` WHERE `tb_view`.`key_code`=`tb_customer_comment`.`status` AND type=5) AS  status 
	
 	FROM `tb_customer_comment` WHERE 1 ";
  $where = '';
